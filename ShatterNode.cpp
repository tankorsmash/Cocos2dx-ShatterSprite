...
void ShatterSprite::run_shatter_action(float duration)
{
    ShatterAction* shatter_action = ShatterAction::create(duration);
    this->setOpacity(0);
    this->runAction(Sequence::createWithTwoActions(shatter_action, cocos2d::RemoveSelf::create()));
}
...
void ShatterSprite::createShatter(){
    ...
    float grid_side_percent = 0.05f;
    this->grid_side_len = std::max(contentSize.width, contentSize.height) * grid_side_percent;
